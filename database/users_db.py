...
class Database:
    ...
    async def get_prev_video(self, current_file_id):
        ...
        if not current:
            return None
        prev_vid = await self.videos.find({"_id": {"$lt": current["_id"]}}).sort("_id", -1).limit(1).to_list(1)
        if prev_vid:
    ...
    async def get_next_video(self, current_file_id):
        ...
        if not current:
            return None
        next_vid = await self.videos.find({"_id": {"$gt": current["_id"]}}).sort("_id", 1).limit(1).to_list(1)
        if next_vid:
