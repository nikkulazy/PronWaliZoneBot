...
class Database:
    ...
    async def is_user_verified(self, user_id):
        ...
        if current_time - pastDate < timedelta(seconds=VERIFY_EXPIRE):
            return True
        return False
    ...
    async def get_verification_stats(self):
        ...
        level1_count = await self.misc.count_documents({"last_verified": {"$gte": midnight_utc}})
        return level1_count

    # ========== NEW METHODS FOR NAVIGATION (using ObjectId) ==========
    async def get_prev_video_id(self, current_file_id):
        """Returns the ObjectId (_id) of previous video, not file_id"""
        current = await self.videos.find_one({"file_id": current_file_id})
        if not current:
            return None
        prev = await self.videos.find({"_id": {"$lt": current["_id"]}}).sort("_id", -1).limit(1).to_list(1)
        if prev:
            return str(prev[0]["_id"])  # return as string for callback data
        return None

    async def get_next_video_id(self, current_file_id):
        """Returns the ObjectId (_id) of next video, not file_id"""
        current = await self.videos.find_one({"file_id": current_file_id})
        if not current:
            return None
        nxt = await self.videos.find({"_id": {"$gt": current["_id"]}}).sort("_id", 1).limit(1).to_list(1)
        if nxt:
            return str(nxt[0]["_id"])
        return None

    async def get_file_id_from_objectid(self, object_id_str: str):
        """Convert ObjectId string back to file_id"""
        try:
            obj_id = ObjectId(object_id_str)
            doc = await self.videos.find_one({"_id": obj_id})
