...
# =========================================================
# 🔙 CALLBACK QUERY HANDLER - ONLY ONE (FIXED)
# =========================================================
@Client.on_callback_query()
async def cb_handler(client: Client, query: CallbackQuery):
    data = query.data
    ...
    # ✅ GET VIDEO BUTTON - PREMIUM CHECK ADD KARO
    elif data == "get_video":
        # ✅ प्रीमियम चेक करो
        is_premium = await db.has_premium_access(user_id)
        
        if not is_premium:
            # अगर प्रीमियम नहीं है तो मैसेज दिखाओ
            await query.answer("⚠️ ये फीचर सिर्फ प्रीमियम यूजर्स के लिए है!", show_alert=True)
            return
        
        # अगर प्रीमियम है तो आगे बढ़ो
        await query.answer("📹 Getting your video...", show_alert=False)
        await handle_video_request(client, query.message)

    # ✅ BRAZZERS BUTTON - PREMIUM CHECK ADD KARO
    elif data == "brazzers":
        # ✅ प्रीमियम चेक करो
        is_premium = await db.has_premium_access(user_id)
