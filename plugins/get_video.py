...
async def handle_video_request_from_callback(client, query: CallbackQuery):
    ...
    # Callback query se data lo
    user_id = query.from_user.id
    username = query.from_user.username or query.from_user.first_name or "Unknown"
    m = query.message  # Query ke message ko use karo
    ...
    if is_premium:
        ...
        if used >= VERIFICATION_DAILY_LIMIT:
            await query.message.reply(limit_reached_msg, reply_markup=buy_button)
            return
        if used >= DAILY_LIMIT:
            if IS_VERIFY:
                verified = await av_x_verification(client, m)
    ...
    if not video_id:
        try:
            ...
            print(f"[Random Video Error] {e}")
            await query.message.reply("❌ Error getting video!")
            return

    if not video_id:
        await query.message.reply("❌ No videos found in the database.")
        return
